...
/**
* Get all available communities (those that users can apply to join)
* 
* @returns Promise with array of community data
*/
export async function getAllCommunities(): Promise<any[]> {
  try {
    const communitiesRef = collection(db, 'communities');
    // Only get active communities
    const q = query(communitiesRef, where('status', '==', 'active'));
    const snapshot = await getDocs(q);
